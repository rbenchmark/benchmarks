...
run <- function(n = 100) {
	...
	isBitZero <- function(pair, bitLoc) {
	    out <- bitAnd(pair, bitPair[bitLoc + 1,]);
	...
	zerocount = function(mask){
	    return(50 - sum(bitAnd(bitPair, mask) > 0));
	}
	...
	floodFill <- function(board, xxx_todo_changeme){
	    ...
	    if (valid(xy) && isBitZero(board, (xy[1] + width*xy[2])) ){
	        ...
	        board <- bitOr(board, bitPair[1 + xy[1] + width*xy[2],]);
	
	        for (f in move){
	            board <- bitOr(board, floodFill(board, f(xy)));
	        }
	    }
	    return(board);
	}
	...
	getBitmask <- function(xy, piece){
	    ...
	    for(cell in piece){
	        xy = move[[cell]](xy);
	        if(valid(xy)){
	            mask <- bitOr(mask, bitPair[1 + xy[1] + width*xy[2],]);
	        }
	        else{
	...
	generateBitmasks <- function(){
	    ...
	    for(piece in pieces){
	        ...
	        while (j >= 0){
	            maskCellAnd <- bitAnd(masks[[j+1]], cellMask);
	...
	solveCell <- function(cell, board){    
	    ...
	    else if (sum(bitAnd(board, bitPair[1 + cell,])) != 0){
	        # Cell full
	        ...
	        for (color in 0:9){
	            if (masks[[color+1]][1] == 0 && masks[[color+1]][2] == 0){
	                for (mask in masksAtCell[[cell+1]][[color+1]]){
	                    if (sum(bitAnd(mask, board)) == 0){
	                        masks[[color+1]] <<- mask;
	                        solveCell(cell-1, bitOr(board, mask));
	                        masks[[color+1]] <<- c(0,0);
	                    }
	                }
	            }
	...
	addSolutions <- function(){
	    ...
	    for(y in 0:(height-1)){
	        for( x in 0:(width-1)) {
	            ...
	            for(color in 0:9){
	                if(sum(bitAnd(masks[[color+1]], mask)) != 0){
	                    s <- append(s, color);
	                    break;
	                }
	                else if (color == 9){
