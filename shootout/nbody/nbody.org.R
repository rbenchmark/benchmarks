...
energy = function (x,v,mass) {
		...
		for (i in 1:(nb-1)){
			distance = sqrt ((x[i,1]-x[(i+1):nb,1])^2+(x[i,2]-x[(i+1):nb,2])^2+(x[i,3]-x[(i+1):nb,3])^2)			
...
for (k in 1:num){
	for (i in 1:(nb-1)){
		...
		v[i,1]=v[i,1]-sum(dx*mass[(i+1):nb]*mag)
		v[i,2]=v[i,2]-sum(dy*mass[(i+1):nb]*mag)
		v[i,3]=v[i,3]-sum(dz*mass[(i+1):nb]*mag)
